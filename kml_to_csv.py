...
def main() -> None:
    ...
    try:
        ...
        kmz_filename: str = 'geopaparazzi_20250721_014514_kmz_20250817_012241.kmz'
        kmz_filepath: str = os.path.join(script_dir, kmz_filename)
        
        if not os.path.exists(kmz_filepath):
            print(f"Input file not found: {kmz_filepath}")
            return

        forms: dict[str, list[ET.Element]] = group_placemarks_by_form(kmz_filepath)
        ...
        try:
            choice_index: int = int(input("\nEnter the number of the form: ")) - 1
            if not 0 <= choice_index < len(form_names):
                print("Invalid choice.")
                return
            selected_form_name: str = form_names[choice_index]
        ...
        first_placemark_description_element: ET.Element | None = selected_placemarks[0].find(
            'kml:description', ns
        )
        ...
        csv_filename: str = f"{selected_form_name.replace(' ', '_').lower()}.csv"
        csv_filepath: str = os.path.join(script_dir, csv_filename)

        with open(csv_filepath, 'w', newline='', encoding='utf-8') as csvfile: # type: ignore
